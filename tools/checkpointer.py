...
def _group_checkpoint_keys(keys: list):
    """
    Group keys based on common prefixes. A prefix is the strings up to the final
    "." in each key.
    Args:
...
def _group_to_str(group: list):
    """
    Format a group of parameter name suffixes into a loggable strings.
    Args:
        group (list[str]): list of parameter name suffixes.
    Returns:
        str: formated strings.
    """
    if len(group) == 0:
        return ""
...
class Checkpointer(object):
    def __init__(
        self, model: nn.Module, save_dir: str = "", *, save_to_disk: bool = True, **checkpointables: object,
    ):
        """
        Args:
            ...
            save_to_disk (bool): if True, save checkpoint to disk, otherwise
                disable saving for this check_pointer.
    ...
    def get_all_checkpoint_files(self):
        ...
        Returns:
            list: All available checkpoint files (.pth files) in target
                directory.
        """
        return [file for file in self.save_dir.iterdir() if file.is_file() and file.suffix == ".pth"]
    ...
    def _convert_ndarray_to_tensor(self, state_dict: dict):
        ...
        # (as returned by Pytorch'strings state_dict()). We should preserve these
        # properties.
        for k in list(state_dict.keys()):
            v = state_dict[k]
            if not isinstance(v, np.ndarray) and not isinstance(v, t.Tensor):
